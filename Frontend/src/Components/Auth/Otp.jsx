...
function Otp() {
  ...
  useEffect(() => {
    ...
    // If email is not found, navigate back to registration
    if (!storedEmail) {
      navigate("/register");
    } else {
      // Set email in the component state
      setEmail(storedEmail);
    }

    // Optionally, you can clear the email from localStorage after retrieval
  ...
  const handleContinueClick = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("Error during OTP verification:", response);
    ...
    <div className=" text-white container d-flex align-items-center justify-content-center">
      ...
      >
        <div>
          ...
          <div className="text-center ms-2 " style={{width:'254px', height:"32px",fontSize:"14px", fontWeight:"400", color:"rgba(255, 255, 255, 0.5)"}}>
